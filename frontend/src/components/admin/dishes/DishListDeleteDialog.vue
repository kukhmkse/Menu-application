...
const props = defineProps(['deleteDish', 'callback'])


const deleteCallback = (entityId) => {
  deleteDialog.value = false
  const promise = dishApi.dishDelete(entityId)
  promise
      .then(() => {
        props.callback()
      })
...
<template>
  <v-dialog v-model="deleteDialog" width="300px">
    <v-card :title="`Delete ${deleteDish.name}`">
      <v-card-item>
        {{ "confirm" }} <strong>{{ deleteDish.name }}</strong>{{ " delete" }}?
      </v-card-item>
      <v-card-actions>
        <div style="width: 100%" class="d-flex flex-row justify-space-between pa-2">
          ...
          <v-btn @click="deleteCallback(deleteDish.id)">
            <span class="shadow" />
            <span class="edge" />
