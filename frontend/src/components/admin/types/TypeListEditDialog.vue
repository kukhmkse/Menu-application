...
const props = defineProps(['editType', 'callback']);
...
const submit = () => {
  ...
  loading.value = true
  const promise = props.editType ? dishTypeApi.dishTypeEdit(props.editType.id, typeData.value) : dishTypeApi.dishTypeCreate(typeData.value)
  promise
      .then(async () => {
        props.callback()
        loading.value = false
...
<template>
  <v-dialog v-model="editDialog" width="400px" style="z-index: 9999;">
    <v-card :title="'Edit type'" class="dialog">
      <LoadingScreen :items="!loading" :error="error">
        <template #content>
          <div class="d-flex flex-column align-center justify-center">
            <v-form :v-if="typeData" style="width: 80%;" @submit.prevent="submit">
              <v-text-field v-if="typeData" v-model="typeData.name" :label="'name'" autofocus clearable
                            variant="solo" class="custom-colored-field" />
              <v-text-field v-if="typeData" v-model="typeData.weight" :label="'weight'" autofocus clearable
