...
const props = defineProps(['deleteType', 'callback'])


const deleteCallback = (entityId) => {
  deleteDialog.value = false
  const promise = dishTypeApi.dishTypeDelete(entityId)
  promise
      .then(() => {
        props.callback()
      })
...
<template>
  <v-dialog v-model="deleteDialog" width="300px">
    <v-card :title="`Delete ${deleteType.name}`">
      <v-card-item>
        {{ "confirm" }} <strong>{{ deleteType.name }}</strong>{{ " delete" }}?
      </v-card-item>
      <v-card-actions>
        <div style="width: 100%" class="d-flex flex-row justify-space-between pa-2">
          ...
          <v-btn @click="deleteCallback(deleteType.id)">
            <span class="shadow" />
            <span class="edge" />
