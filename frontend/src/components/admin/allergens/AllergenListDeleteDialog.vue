...
const props = defineProps(['deleteAllergen', 'callback'])


const deleteCallback = (entityId) => {
  deleteDialog.value = false
  const promise = allergenApi.allergenDelete(entityId)
  promise
      .then(() => {
        props.callback()
      })
...
<template>
  <v-dialog v-model="deleteDialog" width="300px">
    <v-card :title="`Delete ${deleteAllergen.name}`">
      <v-card-item>
        {{ "confirm" }} <strong>{{ deleteAllergen.name }}</strong>{{ " delete" }}?
      </v-card-item>
      <v-card-actions>
        <div style="width: 100%" class="d-flex flex-row justify-space-between pa-2">
          ...
          <v-btn @click="deleteCallback(deleteAllergen.id)">
            <span class="shadow" />
            <span class="edge" />
